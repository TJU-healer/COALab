...
void cpu_exec(volatile uint32_t n) {
	...
	for(; n > 0; n --) {
        ...
        WP *wp = gethead();
        while(wp != NULL){
            int ret = Isequal_expr(wp);    //判断表达式的值是否发生变化
            if(ret == -1){   //Bad Expression!!
                printf("watchpoint %d : Invalid expression!", wp->NO);
				flag = 1;
            }
			else if(ret == 0){ //待检测表达式的值发生变化
                flag = 1;
            }
            wp = wp->next;
        }

        if(flag) temu_state = STOP;  //此时程序因触发监视点而暂停
